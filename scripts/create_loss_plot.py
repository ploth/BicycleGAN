...
def parse_args():
    ...
    arg_parser = argparse.ArgumentParser(description="create loss plot",
...
if __name__ == '__main__':
    ...
    point = re.compile(r'\((epoch):\s*(\d+),\s*(iters):\s*(\d+),\s*(time):\s*(\d*\.\d+),\s*(data):\s*(\d*\.\d+)\)\s*(G_GAN):\s*(\d*.\d+)\s*(D):\s*(\d*\.\d+)\s*(G_GAN2):\s*(\d*\.\d+)\s*(D2):\s*(\d*.\d+)\s*(G_L1):\s*(\d*\.\d+)\s*(G_IL1):\s*(\d*\.\d+)\s*(z_L1):\s*(\d*\.\d+)\s*(kl):\s*(\d*\.\d+)\s*')
    for line in loss_log:
        match = re.match(point, line)
        if match:
            for i in range(1, len(match.groups()), 2):
                values[match[i]].append(match[i + 1])

    data = np.array(values['G_GAN'])
    fig, ax = plt.subplots()
    ax.plot(data)
    fig.show()
